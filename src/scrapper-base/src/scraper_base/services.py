...
class PropertyService:
    ...
    async def upsert_property(
        ...
        Matches on ``(portal_source, source_id)``. If a record exists, it is
        updated and ``last_seen_at`` is refreshed. Otherwise a new record is
        inserted.
        ...
        # Insert new record — PostgreSQL generates the ID via
        # GENERATED BY DEFAULT AS IDENTITY (see migration 0001).
        # For SQLite (tests), use max(id)+1 as a cross-DB fallback.
