...
**Goal**: Address findings from the deep code review — fix race conditions, type mismatches, unused code, and documentation gaps.

**Tasks**:

- [ ] **7.1** Fix TOCTOU race in `services.py` ID generation:
  - Remove manual `max(id) + 1` logic in `PropertyService.upsert_property()`
  - Use PostgreSQL `GENERATED BY DEFAULT AS IDENTITY` auto-generation and `RETURNING` to get assigned IDs
  - Change to `GENERATED ALWAYS AS IDENTITY` if manual override should be prevented
- [ ] **7.2** Fix pipeline session creation in `pipeline.py`:
  - Replace `__import__("sqlalchemy").ext.asyncio.AsyncSession(self._db_engine)` with `self._session_factory()`
  - Import `AsyncSession` at module top instead of using `__import__`
- [ ] **7.3** Align migration and model on PostGIS type:
  - Change `alembic/versions/0001_create_core_tables.py:71` from `GEOGRAPHY(Point, 4326)` to `GEOMETRY(Point, 4326)`
  - Verify model `geoalchemy2.Geometry("POINT", srid=4326)` matches
- [ ] **7.4** Use stable identifier in pipeline:
  - Replace `str(id(self))` in `BasePipeline.__init__()` with `str(uuid.uuid4())` for scraper_id
- [ ] **7.5** Migrate services module to structlog:
  - Change `logging.getLogger(__name__)` to `structlog.get_logger(__name__)` in `services.py`
  - Update all log calls to use structured extra fields
- [ ] **7.6** Remove insecure MinIO defaults:
  - Remove fallback default values for `MINIO_ACCESS_KEY` and `MINIO_SECRET_KEY` in `storage.py`
  - Raise `ValueError` on init if env vars are not set
  - Document setup in README
- [ ] **7.7** Update plan execution log and task checkboxes:
  - Mark Phase 5 tasks (5.1-5.10) as `[x]` since they are implemented
  - Update ruff line-length in plan (Phase 5 task 5.9) from 100 to 120
- [ ] **7.8** Remove or integrate unused `get_db_session()` in `database.py`:
  - If unused, remove the function. If needed, refactor pipeline to use it.
- [ ] **7.9** Add application-level env vars to `.env.example`:
  - Add commented stubs for `MINIO_ENDPOINT`, `MINIO_ACCESS_KEY`, `MINIO_SECRET_KEY`, `MINIO_BUCKET`
- [ ] **7.10** Strengthen test assertions:
  - Fix `test_logging.py:39,59` — use `capsys` or `StringIO` to capture and validate log output
  - Fix `test_services.py:45` — use robust timestamp comparison
- [ ] **7.11** Add partition creation documentation:
  - Add comment/TODO in migration `0001_create_core_tables.py` about adding partitions for new portal sources
- [ ] **7.12** Run verification:
  - `ruff check src/scrapper-base/` — zero warnings
  - `mypy src/scrapper-base/ --strict` — zero errors
  - `pytest src/scrapper-base/tests/ -v --cov=scraper_base --cov-fail-under=80` — all pass
...
- Must: All major findings (M-1 through M-4) resolved
- Must: No hardcoded secrets as defaults
- Must: `ruff check` and `mypy --strict` pass with zero issues
- Must: All existing tests pass
- Must: Plan execution log accurately reflects actual state

**Files and modules**:

- `src/scrapper-base/src/scraper_base/services.py` (fix: ID generation, structlog)
- `src/scrapper-base/src/scraper_base/pipeline.py` (fix: session factory, scraper_id, imports)
- `src/scrapper-base/alembic/versions/0001_create_core_tables.py` (fix: GEOGRAPHY→GEOMETRY)
- `src/scrapper-base/src/scraper_base/storage.py` (fix: remove insecure defaults)
- `src/scrapper-base/src/scraper_base/database.py` (fix: remove dead code or refactor)
- `src/scrapper-base/tests/test_logging.py` (fix: strengthen assertions)
- `src/scrapper-base/tests/test_services.py` (fix: timestamp comparison)
- `.env.example` (fix: add application-level env var stubs)
- `doc/changes/STORY-1/chg-STORY-1-plan.md` (fix: execution log + task checkboxes)

**Tests**:

- Existing test suite must continue to pass
- New tests verifying corrected ID generation are encouraged

**Completion signal**: `fix(STORY-1): remediate code review findings for Phase 7`
...
| 1.1 | 2026-06-20 | reviewer | Address code review findings — Phase 7 remediation, Phase 5 status fix |
...
| Phase 4: Cross-Cutting | completed | 2026-06-20 | 2026-06-20 | — | pipeline, metrics, logging, storage — ruff/mypy clean |
| Phase 5: Tests & Verification | completed | 2026-06-20 | 2026-06-20 | — | 50 tests written and passing (SQLite in-memory), linting/type config added |
| Phase 6: Manual Smoke Test | pending | — | — | — | — |
| Phase 7: Code Review Remediation | pending | — | — | — | — |
