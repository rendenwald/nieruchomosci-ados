...
- [x] **8.1** Create `src/real-estate-api/Dockerfile`:
  - Multi-stage build: `python:3.12-slim` base
  - Install `uv`, copy `pyproject.toml` and `uv.lock`, run `uv sync --no-dev`
  - Copy source code
  - Expose port 8000
  - CMD: `uvicorn app.main:create_app --factory --host 0.0.0.0 --port 8000`
  - Build context is repo root (needs `scrapper-base` as editable dep)
- [x] **8.2** Add `real-estate-api` service to `docker-compose.yml`:
  - Build context: `.` (repo root), dockerfile: `src/real-estate-api/Dockerfile`
  - Port: `8000:8000`
...
- [x] **8.3** Create `src/real-estate-api/.env.example` with all configurable env vars
- [x] **8.4** Create `src/real-estate-api/README.md` with:
  - Setup instructions (uv, virtualenv, docker-compose)
  - Available environment variables
  - How to run tests
  - API documentation link (auto-generated by FastAPI)
  - Full API reference with query params, response shape, project structure
- [x] **8.5** Perform end-to-end verification:
  - `ruff check .` — passes
  - `mypy app/` — passes, no issues in 16 source files
  - `mypy tests/` — passes, no issues in 6 source files
  - `pytest tests/ -v --cov=app` — 33/33 pass, 77% coverage (main.py/redis_client.py have integration-only paths)
...
- [x] **9.1** Perform self-review:
  - ✅ All 10 ACs implemented and tested
  - ✅ F-1 through F-5 capabilities present
  - ✅ NFRs: graceful fallback, timeout handling, error handling all implemented
  - ✅ Prometheus metrics: `cache_hits_total`, `cache_misses_total`, `cache_errors_total`, `cache_operation_duration_seconds`, `cache_entry_size_bytes`
  - ✅ X-Cache header values: `hit` / `miss` / `miss (fallback)`
  - ✅ Concurrent request dedup via `SET NX` lock
  - ✅ Health check with Redis connectivity status
  - ✅ `max_limit=100` enforced
  - ✅ Sort parameter works
- [x] **9.2** Run full verification suite:
  - `ruff check .` — no warnings ✅
  - `mypy app/` — no type errors, 16 source files ✅
  - `mypy tests/` — no type errors, 6 source files ✅
  - `pytest tests/ -v --cov=app` — 33/33 pass, coverage 77% (main.py/redis_client.py have integration-only paths that need real Redis/DB to cover)
- [x] **9.3** Update plan and spec if implementation diverged:
  - No significant divergences from spec or plan
  - Spec used `sort_by` as param name; plan/properties router use `sort_by` — consistent
  - Spec used `miss (fallback)` as cache status value for degraded mode — consistent
  - Spec used `cache_entry_size_bytes` Gauge — present in metrics
- [x] **9.4** Verify version impact:
  - `version_impact: minor` consistent with adding new feature (caching layer) without breaking API contract
  - Version already `0.1.0` in `pyproject.toml` and `app/__init__.py` — no bump needed (initial release)
...
| Phase 8: Docker + Docs | ✅ Complete | 2026-06-22 | 2026-06-22 | (next commit) | Dockerfile exists, docker-compose service added, README.md created, .env.example present |
| Phase 8: Docker + Docs | ✅ Complete | 2026-06-22 | 2026-06-22 | (commit pending) | Dockerfile exists, docker-compose service added, README.md created, .env.example present |
| Phase 9: Review & Release | ✅ Complete | 2026-06-22 | 2026-06-22 | (commit pending) | All ACs verified, ruff/mypy/pytest pass, spec reconciled |
